...
class TicTacToe:
    ...
    def get_best_computer_move(self):
        ...
        print(f"Debug: Evaluating {len(empty_cells)} possible moves")
        ...
        for row, col in empty_cells:
            ...
            print(f"Debug: Move ({row},{col}) scores {score}")

            if score > best_score:
                best_score = score
                best_move = (row, col)

        print(f"Debug: Best move is {best_move} with score {best_score}")
        return best_move
    ...
    def make_computer_move(self):
        ...
        # Debug: show available moves
        empty_cells = self.get_empty_cells()
        print(f"Debug: Available moves: {empty_cells}")

        move = self.get_best_computer_move()
        if move:
            row, col = move
            print(f"Debug: Computer selected move: ({row}, {col})")
            success = self.make_move(row, col)
            if success:
                print(f"Computer chooses row {row}, column {col}")
            else:
                print(f"ERROR: Computer tried invalid move ({row}, {col})")
                # Fallback to first available move
                if empty_cells:
                    row, col = empty_cells[0]
                    self.make_move(row, col)
                    print(f"Computer fallback move: row {row}, column {col}")
        else:
            print("ERROR: No move returned by AI")
